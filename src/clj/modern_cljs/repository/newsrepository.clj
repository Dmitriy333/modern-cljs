...
(defrecord NewsRepositoryImpl [dbconfig]
  ...
  (find-by-id [this id]
    (let [news-item (first (jdbc/query dbconfig ["select * from news where id = ?" id]))]
      (model/->News (news-item :id) (news-item :title) (news-item :short_text) (news-item :full_text) (news-item :creation_date))))
  (find-all [this]
    (let [news-list (jdbc/query dbconfig ["SELECT * from news order by creation_date DESC"])]
      (for [news news-list]
        (model/->News (news :id) (news :title) (news :short_text) (news :full_text) (news :creation_date)))))
  (add [this model]
